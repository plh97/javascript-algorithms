			...
			[].concat(_toConsumableArray(root.querySelectorAll('div')), _toConsumableArray(root.querySelectorAll('span'))).filter(function (dom) {
				...
				_this.opt[i].forEach(function (e, i, arr) {
					...
					[].concat(_toConsumableArray(span.attributes)).filter(function (attr) {
						...
						// span.removeAttribute(attr.localName);
					});
					var content = dom.innerHTML;
					// let reg = new RegExp(`\\{\\{[\s\S]+\\}\\}`,'gmi');
					// content = content.replace(reg,'eval($1)')
		...
		value: function modifyDataToHTML(key, isModify) {
			...
			if (_typeof(this.opt[key]) === 'object') {
				this.opt[key].forEach(function (e, i, arr) {
					...
					root.querySelector('[' + _this2.dep['this.' + key + '.' + i] + ']').style.transform = 'translate(' + arr.indexOf(i + 1) * 120 + '%,' + time * 20 + '%)';
					// root.querySelector(`[${this.dep[`this.${key}.${i}`]}]`).innerHTML = i+1;
				});

				// this.opt[i].forEach((e,i) =>{
				...
				// 	let reg = new RegExp(`\{\{(${variable})\}\}`,'gmi');
